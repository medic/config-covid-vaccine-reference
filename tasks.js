...
module.exports = [
  ...
  {
    name: 'covid.vaccine_followup',
    icon: 'icon-follow-up',
    title: 'covid vaccine followup',
    appliesTo: 'reports',
    ...
    resolvedIf: function (contact, report, event, dueDate) {
      ...
      //reported_date + 1 so that source ds_follow_up does not resolve itself
      const endTime = addDays(dueDate, event.end + 1).getTime();
      return isFormArraySubmittedInWindow(contact.reports, ['covid_vaccine_follow_up'], startTime, endTime);
    },
    actions: [
      {
        type: 'report',
        form: 'covid_vaccine_follow_up',
        modifyContent: function (content) {
          content.is_covid_vaccine_follow_up = 'yes';
        }
      }
    ],
    events: [
      {
        id: 'covid-vaccine-follow-up',
        start: 3,
        end: 7,
        dueDate: function (event, contact, report) {
          return getDateISOLocal(getField(report, 'vaccination_status.next_visit'));
        }
      }
    ]
  },
  ...
  {
    name: 'covid.vaccine_referral',
    icon: 'icon-follow-up',
    title: 'covid vaccine referral',
    appliesTo: 'reports',
    appliesToType: ['covid_vaccine_registration'],
    appliesIf: function (contact, report) {
      return getField(report, 'vaccination_details2.receive_vaccine') === 'yes' && isAlive(contact);
    },
    resolvedIf: function (contact, report, event, dueDate) {
      const startTime = Math.max(addDays(dueDate, -event.start).getTime(), report.reported_date + 1);
      //reported_date + 1 so that source ds_follow_up does not resolve itself
      const endTime = addDays(dueDate, event.end + 1).getTime();
      return isFormArraySubmittedInWindow(contact.reports, ['covid_vaccine_referral_follow_up'], startTime, endTime);
    },
    actions: [
      {
        type: 'report',
        form: 'covid_vaccine_referral_follow_up',
        modifyContent: function (content) {
          content.is_covid_vaccine_referral = 'yes';
        }
      }
    ],
    events: [
      {
        id: 'covid-vaccine-referral-follow-up',
        start: 3,
        end: 7,
        dueDate: function (event, contact, report) {
          return getDateISOLocal(getField(report, 'vaccination_details2.vaccine_follow_up'));
        }
      }
    ]
  }
